...
import RoomPackageInfo from "@/components/ClientSide/room-package-info"
import EnhancedBookingSummary from "@/components/ClientSide/enhanced-booking-summary"
import GuestDetailsForm, { type GuestData } from "@/components/ClientSide/guest-details-form"
import ReviewPayment from "@/components/ClientSide/review-payment"
import { useRoomsDataOnce } from "@/hooks/ClientSide/useRoomsDataStore"
  ...
  const handleProceedPayment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.location.href = "/ClientSide/room";
      } else {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false);
   useEffect(() => {
    checkSession().then(data => {
      setIsLoggedIn(!!data?.loggedIn); // Giả sử API trả về { loggedIn: true/false }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <GuestDetailsForm
                ...
                isLoggedIn={isLoggedIn} // You can check actual login status here
              />
            </div>
            <div className="lg:col-span-1">
              <EnhancedBookingSummary
